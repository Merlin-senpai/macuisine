...
export async function getSession() {
  ...
  await query(
    'UPDATE sessions SET last_accessed = NOW(), expires_at = $1 WHERE id = $2',
...
export async function getUserByEmailOrUsername(identifier: string) {
  const users = await query(
    'SELECT * FROM users WHERE (email = $1 OR username = $1) AND role IN (\'admin\', \'super_admin\')',
    [identifier]
  ) as any[];
...
export async function logUserActivity(
  ...
  await query(
    'INSERT INTO user_activity_log (user_id, action, resource_type, resource_id, ip_address, user_agent) VALUES ($1, $2, $3, $4, $5, $6)',
...
export async function logLoginAttempt(
  ...
  await query(
    'INSERT INTO login_attempts (username, success, ip_address, user_agent) VALUES ($1, $2, $3, $4)',
...
export async function checkLoginAttempts(ipAddress: string, username: string): Promise<boolean> {
  const attempts = await query(
    `SELECT COUNT(*) as count FROM login_attempts 
     WHERE (ip_address = $1 OR username = $2) 
     AND success = FALSE 
